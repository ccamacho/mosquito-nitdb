...
sol: |
  In this case the deployment failed with a
  ControllerLoadBalancerDeployment_Step1 error.
  Check for tab chars in the yaml files also
  validate that all hiera calls are returning data.
  For example, if using:
  zaqar::db::mysql::host: "%{hiera('mysql_virtual_ip')}"
  when the host is empty might fail.
